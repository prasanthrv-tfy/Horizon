...
`src/blog/runner.py` is refactored into four focused modules — `loader`, `enricher`, `scorer`, and `reporter` — while `runner.py` is retained as the CLI entry point delegating to those modules.
