...
The `horizon-publish` CLI SHALL scan `docs/_posts/` for Markdown files produced by `horizon-blog`, collecting all `*.md` files across profile subdirectories.

#### Scenario: Posts found
- **WHEN** `docs/_posts/` contains Markdown files
- **THEN** the CLI SHALL log the count of posts found

#### Scenario: No posts found
- **WHEN** `docs/_posts/` is empty or does not exist
