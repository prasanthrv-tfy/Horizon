...
#### Scenario: Item passes Path A (technical depth)
- **WHEN** an item scores >= threshold on all dimensions in Path A
- **THEN** the item is included, `inclusion_path` is set to "A"

#### Scenario: Item passes Path B (ecosystem event)
- **WHEN** an item fails Path A but scores >= threshold on all dimensions in Path B
...
For each included item, the weighted sum SHALL be computed per path using the path's dimensions and their `path_a_weight` or `path_b_weight` values. The item's final `weighted_sum` is the maximum across all paths it could satisfy.

#### Scenario: Path A item weighted sum computed
- **WHEN** an item is included via Path A
- **THEN** `weighted_sum = Σ(dimension.path_a_weight × score)` for all Path A dimensions

#### Scenario: Path B item weighted sum computed
- **WHEN** an item is included via Path B
- **THEN** `weighted_sum = Σ(dimension.path_b_weight × score)` for all Path B dimensions
