...
**Reason**: Superseded by `scoring_dimensions` and `gate_paths` on `BlogPromptProfile`. The `ranking_context` string is insufficient to drive gate-based filtering with per-dimension thresholds.
**Migration**: Define `scoring_dimensions: List[ScoringDimension]` and `gate_paths: List[List[str]]` on the profile. The `ranking_context` field is deprecated but retained for backwards compatibility — it is ignored when `scoring_dimensions` is non-empty.

### Requirement: rank_by_relevance accepts optional audience context
**Reason**: `rank_by_relevance()` is replaced by `score_items_for_profile()` for profiles with scoring dimensions. The audience context concept is now expressed through `ScoringDimension` definitions and their anchors.
**Migration**: Define `scoring_dimensions` on the profile. `rank_by_relevance()` remains available as a fallback for profiles without scoring dimensions.
